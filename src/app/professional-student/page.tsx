 ...
 * 
 * Real-time, active trading terminal connected to live market session
 * with actual market data, order execution, and session timing.
 */
  ...
  useEffect(() => {
    const initializeSession = async () => {
      try {
        ...
        const sessionResponse = await fetch(`/api/student/session/find?sessionId=session_1762043911884_xlo4m2eux`);
    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center">
        <div className="text-red-400 font-mono text-center">
          <div className="text-xl mb-2">No Active Session</div>
          <div className="text-sm">Please join a trading session first</div>
        </div>
      </div>
    );
  }
    ...
    <div className="h-screen bg-black text-green-400 font-mono text-xs overflow-hidden select-none">
      ...
      <div className="bg-gray-900 border-b border-green-600 px-3 py-1 flex justify-between items-center">
        <div className="flex items-center space-x-6">
          <span className="text-yellow-400 font-bold tracking-wider">HYPERTICK PROFESSIONAL</span>
